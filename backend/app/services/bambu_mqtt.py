...
class BambuMQTTClient:
    ...
    @staticmethod
    def _reason_code_text(rc) -> str:
        """Return a human-readable reason code string."""
        if rc is None:
            return "Unknown"
    ...
    @classmethod
    def _describe_connect_failure(cls, rc) -> str:
        """Return a user-actionable MQTT connect failure message."""
        reason = cls._reason_code_text(rc)
        if reason == "Not authorized":
            return "Not authorized - check printer access code and ensure Developer Mode/LAN mode is enabled"
        if reason == "Bad user name or password":
            return "Bad user name or password - verify the printer access code"
        return reason
    ...
    def _on_connect(self, client, userdata, flags, rc, properties=None):
        if rc == 0:
            ...
            self.state.connected = False
            self.state.connection_error = self._describe_connect_failure(rc)
            logger.warning("[%s] MQTT connect failed: %s", self.serial_number, self.state.connection_error)
            if self.on_state_change:
                self.on_state_change(self.state)
    ...
    def _on_disconnect(self, client, userdata, disconnect_flags=None, rc=None, properties=None):
        ...
        disconnect_reason = self._reason_code_text(rc)
        if self.state.connection_error and disconnect_reason == "Unspecified error":
            logger.info(
                "[%s] MQTT disconnected after failed connect: %s",
                self.serial_number,
